...
void Graphics_RenderSplashScreen(AppContext *app, GameState *game) {
    ...
    Graphics_DrawTextCentered(app, app->fontLarge, "WORDS_COLLIDE", 200, titleColor);
...
void Graphics_RenderMultiplayerMenu(AppContext *app, GameState *game) {
    ...
    Graphics_DrawTextCentered(app, app->fontLarge, "Multiplayer Setup", 150, titleColor);
    Graphics_DrawTextCentered(app, app->fontRegular, "Make sure both devices are on the same Wi-Fi!", 250, (SDL_Color){50, 50, 50, 255});
...
void Graphics_RenderIPInput(AppContext *app, GameState *game) {
    ...
    Graphics_DrawTextCentered(app, app->fontLarge, "Join Game", 150, titleColor);
    Graphics_DrawTextCentered(app, app->fontRegular, "Enter the Host's IP Address:", 250, promptColor);
    ...
    if (strlen(game->targetIP) > 0) {
        ...
        Graphics_DrawText(app, app->fontRegular, game->targetIP, input_box_rect.x + 10, input_box_rect.y + 10, textColor);
    }
    ...
    Graphics_DrawTextCentered(app, app->fontRegular, "Press ENTER to Connect", 400, promptColor);
    Graphics_DrawButton(app, &game->backButton); 
